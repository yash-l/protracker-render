...
logger = logging.getLogger("Spectre.Flawless")
...
# --- UI TEMPLATE (FLAWLESS RESPONSIVE) ---
HTML_BASE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        :root {
            ...
            /* SIZING VARS */
            --sidebar-width: 260px;
            --sidebar-mini: 60px; /* Ultra thin for mobile */
        }
        ...
        * { box-sizing: border-box; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        h1 { font-family: 'Cinzel', serif; font-size: 2rem; margin: 0 0 0.5rem 0; color: var(--accent); line-height: 1.1; }
        ...
        /* TABLE */
        .table-wrap { width: 100%; overflow-x: auto; background: var(--surface); border: 1px solid var(--border); }
        ...
        /* FORMS & INPUTS (BLUE REMOVAL HACK) */
        input, select, textarea {
            width: 100%; padding: 14px; border: 1px solid var(--border); background: #FCFAF5;
            font-family: 'Lora', serif; font-size: 16px; color: var(--text-main); border-radius: 0;
            -webkit-appearance: none;
        }
        /* Override Chrome Autofill Blue */
        ...
        label { display: block; font-size: 0.65rem; font-family: 'Cinzel', serif; margin-bottom: 0.6rem; color: var(--text-muted); letter-spacing: 0.1em; font-weight: 600; }
        .form-group { margin-bottom: 1.5rem; }

        .btn {
            background: var(--accent); color: var(--gold); padding: 14px 20px; border: 1px solid var(--accent);
        ...
        @media (max-width: 900px) {
            /* Shrink Sidebar to Green Line */
            .sidebar { width: var(--sidebar-mini); padding: 2rem 0.5rem; align-items: center; border-right: none; box-shadow: 4px 0 10px rgba(0,0,0,0.1); }
            
            /* Hide Sidebar Text */
            .brand span, .menu-label, .nav-item span, .user-info, .logout-btn { display: none; }
            
            /* Center Icons */
            .brand i { margin: 0; font-size: 1.5rem; color: var(--gold); }
            .nav-item { justify-content: center; padding: 1rem; width: 100%; }
            .nav-item i { margin: 0; font-size: 1.3rem; }
            .user-panel { justify-content: center; width: 100%; padding-top: 1rem; }
            
            /* Adjust Main Content */
            .main { margin-left: var(--sidebar-mini); padding: 2rem 1.2rem; width: calc(100% - var(--sidebar-mini)); }
            ...
            /* Login Mobile Fix */
            .login-box { padding: 2rem !important; width: 90% !important; max-width: 100%; }
        }
        
        .status-badge { padding: 4px 8px; border-radius: 0; font-size: 0.6rem; font-family: 'Cinzel', serif; text-transform: uppercase; border: 1px solid; }
        ...
        .search-bar { position: relative; width: 100%; max-width: 250px; }
        .search-bar input { padding-left: 2.5rem; border-bottom: 1px solid var(--border); border-top: none; border-left: none; border-right: none; background: transparent; }
        .search-bar input:focus { border-bottom: 1px solid var(--gold); }
        .search-bar i { position: absolute; left: 0.5rem; top: 50%; transform: translateY(-50%); color: var(--gold); }

        .login-wrap { display: flex; height: 100vh; align-items: center; justify-content: center; padding: 1rem; }
        .login-box { width: 100%; max-width: 420px; padding: 3rem; background: var(--surface); border: 1px solid var(--gold); box-shadow: 0 10px 30px rgba(0,0,0,0.05); position: relative; }
        .login-box::before { content: ''; position: absolute; top: 6px; left: 6px; right: 6px; bottom: 6px; border: 1px solid var(--border); pointer-events: none; }
    </style>
</head>
<body>
    {% if not hide_sidebar %}
    <div class="sidebar">
        ...
        <div class="menu">
            ...
            <a href="/dashboard" class="nav-item {{ 'active' if active_page == 'dashboard' else '' }}">
                <i class="fas fa-columns"></i><span class="nav-text">DASHBOARD</span>
            </a>
            <a href="/intelligence" class="nav-item {{ 'active' if active_page == 'intelligence' else '' }}">
                <i class="fas fa-network-wired"></i><span class="nav-text">INTELLIGENCE</span>
            </a>
            <a href="/logs" class="nav-item {{ 'active' if active_page == 'logs' else '' }}">
                <i class="fas fa-history"></i><span class="nav-text">ARCHIVE</span>
            </a>
            
            <div class="menu-label">System</div>
            <a href="/settings" class="nav-item {{ 'active' if active_page == 'settings' else '' }}">
                <i class="fas fa-sliders-h"></i><span class="nav-text">CONFIG</span>
            </a>
            {% if session.get('is_admin') %}
            <a href="/users" class="nav-item {{ 'active' if active_page == 'users' else '' }}">
                <i class="fas fa-users-cog"></i><span class="nav-text">MEMBERS</span>
            </a>
            {% endif %}
        </div>
...
async def dashboard():
    ...
    for t in targets:
        ...
        <tr style="cursor:pointer; transition:background 0.2s;" onclick="window.location='/target/{t['id']}'">
            <td>
                <div style="font-weight:600; color:var(--text-main); font-family:'Cormorant Garamond', serif; font-size:1.1rem;">{t['display_name']} {alert}</div>
    ...
    <div class="header-flex">
        <div>
            ...
            <div class="subtitle" style="margin-bottom:0;">SURVEILLANCE OPERATIONS OVERVIEW</div>
        </div>
        <a href="/add" class="btn" style="width:auto;"><i class="fas fa-plus"></i> NEW TARGET</a>
    </div>
    
    <div class="stats-grid">
        <div class="stat-card"><div class="stat-label">TOTAL TARGETS</div><div class="stat-val">{total_count}</div><div style="font-size:0.7rem; color:var(--text-muted); font-style:italic;">QUOTA: {me['max_targets']} ALLOWED</div></div>
        <div class="stat-card"><div class="stat-label">ACTIVE UPLINKS</div><div class="stat-val" style="color:var(--success);">{online_count}</div><div style="font-size:0.7rem; color:var(--text-muted); font-style:italic;">CURRENTLY ONLINE</div></div>
    ...
    <div class="table-wrap">
        <div style="padding:1.5rem; display:flex; justify-content:space-between; align-items:center; border-bottom:1px solid var(--border); flex-wrap:wrap; gap:10px;">
...
async def intelligence():
    ...
    <div class="stat-card">
        <form method="POST" style="display:flex; gap:1rem; align-items:flex-end; flex-wrap:wrap;">
            ...
            <button class="btn" style="height:50px; margin-top:1rem;">RUN ANALYSIS</button>
        </form>
    </div>
    {result_html}"""
...
async def target_detail(t_id):
    ...
    <div class="header-flex">
        ...
        <div style="display:flex; gap:10px; margin-top:1rem; flex-wrap:wrap;">
            {alert_btn}
            <a href="/export/{t_id}" class="btn-outline" style="text-decoration:none;">EXPORT</a>
            <a href="/delete/{t_id}" class="btn" style="background:#fff; border-color:#e5e7eb; color:var(--danger);">DELETE</a>
        </div>
    </div>
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:2rem;">
        <div class="table-wrap">
            <div style="padding:1.5rem; border-bottom:1px solid var(--border); font-family:'Cinzel', serif; font-weight:700; font-size:0.7rem; color:var(--accent);">SESSION LOG</div>
