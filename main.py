...
logger = logging.getLogger("Netrunner.Trial")
...
async def init_db():
    async with await get_db() as db:
        ...
        # --- MIGRATIONS ---
        try: await db.execute("SELECT owner_id FROM targets LIMIT 1")
        except: await db.execute("ALTER TABLE targets ADD COLUMN owner_id INTEGER DEFAULT 1")
        try: await db.execute("SELECT max_targets FROM users LIMIT 1")
        except:
            await db.execute("ALTER TABLE users ADD COLUMN max_targets INTEGER DEFAULT 3")
...
async def login():
    ...
    <div class="card" style="max-width:400px; margin: 100px auto; text-align:center;">
        ...
        <div style="margin-top:20px; font-size:0.8rem;">
            <a href="/register" style="color:var(--neon-blue);">CREATE ACCOUNT (14 DAY TRIAL)</a>
        </div>
    </div>
    """
...
async def dashboard():
    ...
    for t in targets:
        ...
        rows += f"""<div class="card grid-item" onclick="location.href='/target/{t['id']}'" style="cursor:pointer;"><div><div style="font-size:1.1rem; font-weight:bold; color:#fff;">{t['display_name']} {owner_badge}</div><div style="font-size:0.8rem; color:var(--neon-blue);">{ident}</div></div><div style="text-align:right;"><span class="status-badge {cls}">{t['last_status'].upper()}</span><div style="font-size:0.7rem; color:#666; margin-top:5px;">{ts}</div></div></div>"""
...
async def add():
    ...
    if request.method == 'POST':
        ...
        async with await get_db() as db:
            await db.execute("INSERT INTO targets (owner_id, tg_id, tg_username, display_name, last_status) VALUES (?,?,?,?, 'unknown')", (uid, tg_id, tg_user, f.get('name')))
            await db.commit()
        return redirect('/dashboard')
