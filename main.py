...
async def init_db():
    async with await get_db() as db:
        ...
        async def add_col(table, col, dtype):
            try: await db.execute(f"SELECT {col} FROM {table} LIMIT 1")
            except: 
...
async def dashboard():
    ...
    # FIX: Use dictionary index AND integer matching
    current_count = len([t for t in targets if t['owner_id'] == uid])
