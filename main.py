...
async def init_db():
    async with await get_db() as db:
        ...
        # --- ROBUST MIGRATION (Fixes "Internal Server Error") ---
        # 1. Targets Table Migrations
        ...
        try: await db.execute("SELECT tg_username FROM targets LIMIT 1")
        except: await db.execute("ALTER TABLE targets ADD COLUMN tg_username TEXT")
        
        try: await db.execute("SELECT phone FROM targets LIMIT 1")
        except: await db.execute("ALTER TABLE targets ADD COLUMN phone TEXT")
        
        try: await db.execute("SELECT pic_path FROM targets LIMIT 1")
        except: await db.execute("ALTER TABLE targets ADD COLUMN pic_path TEXT")

        # 2. Users Table Migrations
        try: await db.execute("SELECT max_targets FROM users LIMIT 1")
        except: await db.execute("ALTER TABLE users ADD COLUMN max_targets INTEGER DEFAULT 3")
        
        try: await db.execute("SELECT expiry_date FROM users LIMIT 1")
        except: await db.execute("ALTER TABLE users ADD COLUMN expiry_date DATETIME")
...
async def register():
    ...
    if request.method == 'POST':
        ...
        # LOGIC: 14 Day Expiry & 3 Targets Limit (Preserved)
...
async def add():
    ...
    if request.method == 'POST':
        ...
        phone = f.get('phone', '') # Safe GET
        tg_id = int(tg_in) if tg_in.isdigit() else 0
        tg_user = tg_in if not tg_in.isdigit() else None
        async with await get_db() as db:
            await db.execute("INSERT INTO targets (owner_id, tg_id, tg_username, phone, display_name, last_status) VALUES (?,?,?,?, ?, 'unknown')", (uid, tg_id, tg_user, phone, f.get('name')))
            await db.commit()
        return redirect('/dashboard')
    content = """<div class="card"><h2>ADD TARGET</h2><form method="POST"><label>Name</label><input name="name" required placeholder="Target Alias"><label>Telegram ID or Username</label><input name="tg_input" required placeholder="@username or 12345678"><label>Phone (Optional)</label><input name="phone" placeholder="+91..."><button type="submit">INITIATE TRACKING</button></form></div>"""
    return await render_template_string(HTML_BASE.replace('{{ CONTENT }}', content))
