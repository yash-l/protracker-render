...
# --- UI TEMPLATE (RESPONSIVE HERITAGE THEME) ---
HTML_BASE = """
<!DOCTYPE html>
<html lang="en">
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; outline: none; }
        ...
        @media (max-width: 900px) {
            ...
            .sidebar { width: 100% !important; min-height: auto !important; padding: 1.5rem !important; border-right: none !important; border-bottom: 4px solid var(--gold); }
            .main { padding: 2rem 1.5rem !important; }
            .stats-grid { grid-template-columns: 1fr !important; }
            .menu { flex-direction: row; flex-wrap: wrap; justify-content: center; gap: 1rem; }
            .nav-item span { display: none; } /* Hide text on mobile nav, keep icons */
            .nav-item i { font-size: 1.2rem !important; }
            .brand { margin-bottom: 1.5rem !important; border-bottom: none !important; padding-bottom: 0 !important; }
        ...
        .menu-label { 
            font-family: 'Cinzel', serif; font-size: 0.65rem; color: rgba(224, 216, 200, 0.5); 
            text-transform: uppercase; letter-spacing: 0.2em; margin: 1.5rem 0 0.5rem 0;
        }
        ...
        .nav-item:hover, .nav-item.active { background: rgba(197, 160, 89, 0.1); color: var(--gold); border-color: rgba(197, 160, 89, 0.2); }
        .nav-item i { width: 20px; text-align: center; font-size: 0.9rem; }

        .user-panel {
            border-top: 1px solid rgba(197, 160, 89, 0.3);
            padding-top: 2rem; margin-top: auto; display: flex; align-items: center; gap: 12px;
        }
        .avatar { 
            width: 38px; height: 38px; background: var(--gold); color: var(--sidebar-bg);
        ...
        h1 { 
            ...
            color: var(--accent); font-weight: 400; letter-spacing: 0.02em; line-height: 1.2;
        }
        .subtitle { 
            color: var(--text-muted); font-size: 0.9rem; margin-bottom: 3rem; 
            font-style: italic; font-family: 'Lora', serif; border-left: 3px solid var(--gold); padding-left: 1rem;
        }
        ...
        .stat-card::after { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 4px; background: var(--accent); }
        ...
        .table-container { 
            ...
            box-shadow: var(--shadow); overflow-x: auto; /* Fix for mobile table overflow */
        }
        table { width: 100%; border-collapse: collapse; min-width: 600px; /* Ensure table doesn't squish */ }
        th { 
            text-align: left; padding: 1.2rem; background: #F9F7F2; border-bottom: 2px solid var(--border); 
            font-family: 'Cinzel', serif; font-size: 0.7rem; font-weight: 700; color: var(--text-main); letter-spacing: 0.1em; 
        }
        td { 
            padding: 1.2rem; border-bottom: 1px solid var(--border); font-size: 0.9rem; color: var(--text-muted); 
            font-family: 'Cormorant Garamond', serif; font-weight: 600; white-space: nowrap;
        }
        tr:hover td { background: #FCFAF5; color: var(--accent); }

        /* INPUTS & BUTTONS */
        .btn {
            background: var(--accent); color: var(--gold); padding: 0.9rem 2rem; border: 1px solid var(--accent);
            text-decoration: none; font-size: 0.75rem; font-weight: 700; display: inline-flex; align-items: center; gap: 10px; 
            cursor: pointer; font-family: 'Cinzel', serif; letter-spacing: 0.1em; text-transform: uppercase; transition: all 0.3s;
        }
        .btn:hover { background: var(--gold); color: var(--accent); border-color: var(--gold); }
        
        .btn-outline { background: transparent; border: 1px solid var(--text-muted); color: var(--text-main); padding: 0.6rem 1.2rem; }
        .btn-outline:hover { border-color: var(--gold); color: var(--gold); }
        
        input, select, textarea {
            width: 100%; padding: 1rem; border: 1px solid var(--border); background: #FCFAF5;
            font-family: 'Lora', serif; font-size: 1rem; color: var(--text-main); transition: 0.3s;
        }
        input:focus { border-color: var(--gold); background: #fff; }
        label { display: block; font-size: 0.7rem; font-family: 'Cinzel', serif; margin-bottom: 0.8rem; color: var(--text-muted); letter-spacing: 0.1em; }
        .form-group { margin-bottom: 1.5rem; }

        /* UTILS */
        .status-badge { padding: 5px 12px; font-size: 0.65rem; font-family: 'Cinzel', serif; text-transform: uppercase; display: inline-block; letter-spacing: 0.1em; border: 1px solid; }
        ...
        /* LOGIN */
        .login-wrap { display: flex; height: 100vh; align-items: center; justify-content: center; background: var(--bg); padding: 1rem; }
        .login-box { 
            width: 100%; max-width: 450px; padding: 3rem; background: var(--surface); border: 1px solid var(--gold); 
            box-shadow: 0 20px 40px rgba(26, 47, 35, 0.15); position: relative;
        }
        .login-box::before { content: ''; position: absolute; top: 8px; left: 8px; right: 8px; bottom: 8px; border: 1px solid var(--border); pointer-events: none; }
    </style>
</head>
<body>
    {% if not hide_sidebar %}
    <div class="sidebar">
        ...
        <div class="menu">
            ...
            <a href="/intelligence" class="nav-item {{ 'active' if active_page == 'intelligence' else '' }}"><i class="fas fa-chess-board"></i> <span>INTELLIGENCE</span></a>
            <a href="/logs" class="nav-item {{ 'active' if active_page == 'logs' else '' }}"><i class="fas fa-scroll"></i> <span>ARCHIVE</span></a>
            
            <div class="menu-label">Protocol</div>
            <a href="/settings" class="nav-item {{ 'active' if active_page == 'settings' else '' }}"><i class="fas fa-cogs"></i> <span>CONFIG</span></a>
            {% if session.get('is_admin') %}
            <a href="/users" class="nav-item {{ 'active' if active_page == 'users' else '' }}"><i class="fas fa-users"></i> <span>MEMBERS</span></a>
            {% endif %}
        </div>
...
async def target_detail(t_id):
    ...
    alert_btn = f"""<a href="?toggle_alert=1" class="btn btn-outline" style="color:{'var(--success)' if t['notifications'] else 'var(--text-main)'}; border-color:{'var(--success)' if t['notifications'] else 'var(--text-muted)'};"><i class="fas fa-bell"></i> {'NOTIFICATIONS ON' if t['notifications'] else 'ENABLE ALERTS'}</a>"""
