...
logger = logging.getLogger("Spectre.Mobile")
...
<head>
    ...
    <style>
        :root {
            ...
            --success: #2D5A27;
            --danger: #8B0000;
            --shadow: 0 4px 15px rgba(26, 47, 35, 0.08);
            --sidebar-width: 260px;
            --sidebar-mini: 70px;
        }
        ...
        * { box-sizing: border-box; outline: none; -webkit-tap-highlight-color: transparent; }
        ...
        body { 
            background: var(--bg); color: var(--text-main); font-family: 'Lora', serif; 
        ...
        .brand {
            ...
            margin-bottom: 3rem; color: var(--gold); text-decoration: none; letter-spacing: 0.1em;
            display: flex; align-items: center; white-space: nowrap;
        }
        .brand i { margin-right: 15px; font-size: 1.4rem; min-width: 30px; text-align: center; }

        .menu { display: flex; flex-direction: column; gap: 0.5rem; flex: 1; }
        .menu-label { 
            font-family: 'Cinzel', serif; font-size: 0.6rem; color: rgba(224, 216, 200, 0.5); 
            text-transform: uppercase; letter-spacing: 0.2em; margin: 1rem 0 0.5rem 5px; white-space: nowrap;
        }
        
        .nav-item {
            padding: 0.8rem; color: #E0D8C8; text-decoration: none;
            font-size: 0.85rem; font-weight: 500; display: flex; align-items: center; gap: 15px;
            transition: all 0.2s; border-radius: 6px; white-space: nowrap;
        }
        .nav-item:hover, .nav-item.active { background: rgba(197, 160, 89, 0.15); color: var(--gold); }
        .nav-item i { width: 24px; text-align: center; font-size: 1.1rem; }

        .user-panel {
            border-top: 1px solid rgba(197, 160, 89, 0.3); padding-top: 1.5rem; margin-top: auto;
            display: flex; align-items: center; gap: 12px; white-space: nowrap;
        }
        .avatar { 
            min-width: 36px; height: 36px; background: var(--gold); color: var(--sidebar-bg);
            border-radius: 50%; display: flex; align-items: center; justify-content: center; font-family: 'Cinzel', serif; font-weight: 700;
        }

        /* --- MAIN CONTENT --- */
        .main {
            flex: 1; margin-left: var(--sidebar-width); padding: 3rem 4rem;
            transition: margin-left 0.3s ease; width: 100%;
        }

        h1 { font-family: 'Cinzel', serif; font-size: 2rem; margin: 0 0 0.5rem 0; color: var(--accent); }
        .subtitle { color: var(--text-muted); font-size: 0.9rem; margin-bottom: 2.5rem; font-style: italic; border-left: 3px solid var(--gold); padding-left: 1rem; }
        ...
        .stat-card::after { content: ''; position: absolute; top: 0; left: 0; width: 100%; height: 3px; background: var(--accent); }
        .stat-val { font-size: 2rem; margin: 0.5rem 0; font-family: 'Cormorant Garamond', serif; color: var(--accent); }
        .stat-label { font-size: 0.65rem; text-transform: uppercase; color: var(--text-muted); font-family: 'Cinzel', serif; letter-spacing: 0.1em; }

        /* TABLE */
        .table-wrap { overflow-x: auto; background: var(--surface); border: 1px solid var(--border); box-shadow: var(--shadow); }
        table { width: 100%; border-collapse: collapse; min-width: 500px; }
        th { text-align: left; padding: 1rem; background: #F9F7F2; border-bottom: 2px solid var(--border); font-family: 'Cinzel', serif; font-size: 0.65rem; letter-spacing: 0.1em; color: var(--text-main); }
        td { padding: 1rem; border-bottom: 1px solid var(--border); font-size: 0.9rem; }
        tr:hover td { background: #FCFAF5; }

        /* FORMS & INPUTS */
        input, select, textarea {
            width: 100%; padding: 12px; border: 1px solid var(--border); background: #FCFAF5;
            font-family: 'Lora', serif; font-size: 1rem; color: var(--text-main); border-radius: 4px;
        }
        ...
        input:-webkit-autofill:active {
            -webkit-box-shadow: 0 0 0 30px #FCFAF5 inset !important;
            -webkit-text-fill-color: var(--text-main) !important;
        }
        
        label { display: block; font-size: 0.65rem; font-family: 'Cinzel', serif; margin-bottom: 0.5rem; color: var(--text-muted); letter-spacing: 0.1em; font-weight: 600; }
        .form-group { margin-bottom: 1.5rem; }

        .btn {
            background: var(--accent); color: var(--gold); padding: 12px 20px; border: 1px solid var(--accent);
            text-decoration: none; font-size: 0.75rem; font-weight: 700; display: inline-flex; align-items: center; justify-content: center; gap: 10px; 
            cursor: pointer; font-family: 'Cinzel', serif; letter-spacing: 0.1em; text-transform: uppercase; width: 100%;
        }
        
        .btn-outline { background: transparent; border: 1px solid var(--text-muted); color: var(--text-main); padding: 8px 16px; width: auto; }

        /* MOBILE OPTIMIZATIONS (The Fix) */
        @media (max-width: 900px) {
            .sidebar { width: var(--sidebar-mini); padding: 2rem 0.5rem; align-items: center; }
            .brand span, .menu-label, .nav-item span, .user-info, .logout-btn { display: none; }
            .brand i { margin: 0; font-size: 1.5rem; }
            .nav-item { justify-content: center; padding: 1rem; }
            .nav-item i { margin: 0; font-size: 1.3rem; }
            .user-panel { justify-content: center; width: 100%; }
            
            .main { margin-left: var(--sidebar-mini); padding: 2rem 1.5rem; }
            
            .stats-grid { grid-template-columns: 1fr; gap: 1rem; }
            h1 { font-size: 1.8rem; }
            
            /* Login Page Mobile */
            .login-box { padding: 2rem !important; width: 90% !important; }
        }
        
        .status-badge { padding: 4px 8px; border-radius: 4px; font-size: 0.6rem; font-family: 'Cinzel', serif; text-transform: uppercase; border: 1px solid; }
        .on { color: var(--success); border-color: var(--success); background: rgba(45, 90, 39, 0.05); }
        .off { color: #999; border-color: #ccc; }

        /* Login Specific */
        .login-wrap { display: flex; height: 100vh; align-items: center; justify-content: center; padding: 1rem; }
        .login-box { width: 100%; max-width: 420px; padding: 3rem; background: var(--surface); border: 1px solid var(--gold); box-shadow: 0 10px 30px rgba(0,0,0,0.1); }
    </style>
</head>
<body>
    {% if not hide_sidebar %}
    <div class="sidebar">
        <a href="/dashboard" class="brand"><i class="fas fa-chess-knight"></i><span>SPECTRE</span></a>
        
        <div class="menu">
            <div class="menu-label">Operations</div>
            <a href="/dashboard" class="nav-item {{ 'active' if active_page == 'dashboard' else '' }}"><i class="fas fa-columns"></i><span>DASHBOARD</span></a>
            <a href="/intelligence" class="nav-item {{ 'active' if active_page == 'intelligence' else '' }}"><i class="fas fa-network-wired"></i><span>INTELLIGENCE</span></a>
            <a href="/logs" class="nav-item {{ 'active' if active_page == 'logs' else '' }}"><i class="fas fa-history"></i><span>ARCHIVE</span></a>
            
            <div class="menu-label">System</div>
            <a href="/settings" class="nav-item {{ 'active' if active_page == 'settings' else '' }}"><i class="fas fa-sliders-h"></i><span>CONFIG</span></a>
            {% if session.get('is_admin') %}
            <a href="/users" class="nav-item {{ 'active' if active_page == 'users' else '' }}"><i class="fas fa-users-cog"></i><span>MEMBERS</span></a>
            {% endif %}
        </div>
        ...
        <div class="user-panel">
            ...
            <div class="user-info" style="margin-left:10px;">
                <div style="font-family:'Cinzel', serif; font-size:0.7rem; color:var(--gold);">AGENT {{ session.get('username', 'UNK').upper() }}</div>
            </div>
            <a href="/logout" class="logout-btn" style="color:var(--gold); margin-left:auto;"><i class="fas fa-sign-out-alt"></i></a>
        </div>
    </div>
    {% endif %}
...
async def login():
    ...
    <div class="login-wrap">
        <div class="login-box">
            <h1 style="text-align:center; font-size:2rem; margin-bottom:0.5rem; color:var(--accent);">SPECTRE</h1>
            <p style="color:var(--text-muted); margin-bottom:2rem; text-align:center; font-style:italic;">Private Intelligence Ledger</p>
            {f'<div style="color:var(--danger); margin-bottom:1rem; text-align:center; font-size:0.8rem;">{msg}</div>' if msg else ''}
            <form method="POST">
                <div class="form-group"><label>AGENT IDENTITY</label><input name="username" required></div>
                <div class="form-group"><label>SECURE PASSPHRASE</label><input type="password" name="password" required></div>
                <button class="btn">AUTHORIZE ENTRY</button>
            </form>
            <div style="margin-top:2rem; text-align:center;">
                <a href="/register" style="color:var(--text-muted); font-size:0.7rem; text-decoration:none; font-family:'Cinzel', serif; border-bottom:1px solid var(--gold);">REQUEST NEW IDENTITY</a>
            </div>
        </div>
    </div>
    """
...
async def register():
    ...
    <div class="login-wrap">
        <div class="login-box">
            ...
            <p style="color:var(--text-muted); margin-bottom:2rem; text-align:center;">Create your secure dossier.</p>
            {f'<div style="color:var(--danger); margin-bottom:1rem; text-align:center;">{msg}</div>' if msg else ''}
            <form method="POST">
                ...
                <button class="btn">ESTABLISH PROFILE</button>
            </form>
            <div style="margin-top:2rem; text-align:center;"><a href="/login" style="color:var(--text-muted); font-size:0.7rem; text-decoration:none;">RETURN TO LOGIN</a></div>
        </div>
    </div>"""
    return await render_template_string(HTML_BASE.replace('{{ CONTENT }}', content), hide_sidebar=True)
...
async def dashboard():
    ...
    for t in targets:
        ...
        rows += f"""
        <tr style="cursor:pointer;" onclick="window.location='/target/{t['id']}'">
            <td>
                <div style="font-weight:600; color:var(--text-main); font-family:'Cormorant Garamond', serif; font-size:1.1rem;">{t['display_name']}</div>
                <div class="mono" style="font-size:0.75rem;">{ident}</div>
            </td>
            <td><span class="status-badge {badge}">{t['last_status']}</span></td>
            <td class="mono" style="font-size:0.9rem;">{fmt_time(t['last_seen'])}</td>
            <td style="text-align:right;"><i class="fas fa-chevron-right" style="color:#d8d4c8;"></i></td>
        </tr>"""
    if not rows: rows = "<tr><td colspan='4' style='text-align:center; padding:3rem; color:var(--text-muted); font-style:italic;'>No active intelligence found.</td></tr>"

    content = f"""
    <div style="display:flex; justify-content:space-between; align-items:flex-end; margin-bottom:2rem; flex-wrap:wrap; gap:1rem;">
        <div><h1>Mission Control</h1><div class="subtitle" style="margin-bottom:0;">SURVEILLANCE OPERATIONS</div></div>
        <a href="/add" class="btn" style="width:auto;"><i class="fas fa-plus"></i> NEW TARGET</a>
    </div>
    
    <div class="stats-grid">
        <div class="stat-card"><div class="stat-label">TOTAL TARGETS</div><div class="stat-val">{len(targets)}</div><div style="font-size:0.7rem; color:var(--text-muted);">QUOTA: {me['max_targets']}</div></div>
        <div class="stat-card"><div class="stat-label">ACTIVE UPLINKS</div><div class="stat-val" style="color:var(--success);">{online_count}</div><div style="font-size:0.7rem; color:var(--text-muted);">ONLINE NOW</div></div>
        <div class="stat-card"><div class="stat-label">STATUS</div><div class="stat-val" style="font-size:1.5rem; margin:1rem 0;">{'OPERATIONAL' if cyber_bot.tracking_active else 'OFFLINE'}</div>{f'<a href="/connect" style="font-size:0.7rem; font-weight:700; color:var(--accent);">RECONNECT &rarr;</a>' if not cyber_bot.tracking_active else '<span style="font-size:0.6rem; color:var(--success);">● ACTIVE</span>'}</div>
    </div>
...
async def intelligence():
    ...
    if request.method == 'POST':
        ...
        if t1 and t2:
            ...
            result_html = f"""<div class="stat-card" style="border-left:4px solid var(--gold); margin-top:2rem;">
    ...
    content = f"""<h1>Co-Incidence Engine</h1><div class="subtitle">Analyze behavioral intersections.</div>
    <div class="stat-card">
        <form method="POST" style="display:flex; gap:1rem; flex-wrap:wrap;">
            <div style="flex:1; min-width:200px;"><label>SUBJECT A</label><select name="t1">{opts}</select></div>
            <div style="flex:1; min-width:200px;"><label>SUBJECT B</label><select name="t2">{opts}</select></div>
            <button class="btn" style="width:100%;">RUN ANALYSIS</button>
        </form>
    </div>
    {result_html}"""
...
async def settings():
    ...
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:2rem;">
        <div class="stat-card">
            ...
            <div style="color:var(--success); margin-bottom:1rem; font-size:0.7rem;">{msg}</div>
            <form method="POST">
        ...
        <div class="stat-card">
            ...
            <div style="margin-top:1.5rem;">
                <div style="font-size:0.8rem; color:var(--text-muted); font-family:'Cinzel', serif;">UPLINK STATE</div>
                <div style="font-size:1.5rem; font-weight:500; color:{'var(--success)' if cyber_bot.tracking_active else 'var(--danger)'}; margin:0.5rem 0;">{'CONNECTED' if cyber_bot.tracking_active else 'DISCONNECTED'}</div>
                <a href="/connect" class="btn-outline" style="text-decoration:none;">RESET UPLINK</a>
            </div>
        </div>
    </div>"""
...
async def target_detail(t_id):
    ...
    sess_rows = "".join([f"<tr><td class='mono' style='font-size:0.85rem;'>{fmt_time(x['start_time'])}</td><td class='mono' style='font-size:0.85rem;'>{fmt_time(x['end_time'])}</td><td class='mono' style='color:var(--text-main); font-style:normal;'>{x['duration'] or 'ACTIVE'}</td></tr>" for x in s])
    alert_btn = f"""<a href="?toggle_alert=1" class="btn-outline" style="text-decoration:none; color:{'var(--success)' if t['notifications'] else 'var(--text-main)'}; border-color:{'var(--success)' if t['notifications'] else 'var(--text-muted)'};">{'NOTIFICATIONS ON' if t['notifications'] else 'ENABLE ALERTS'}</a>"""

    content = f"""
    <div style="margin-bottom:2rem;"><a href="/dashboard" style="color:var(--text-muted); text-decoration:none; font-size:0.75rem; font-family:'Cinzel', serif;">&larr; BACK TO DASHBOARD</a></div>
    <div style="display:flex; justify-content:space-between; align-items:start; margin-bottom:2rem; flex-wrap:wrap; gap:1rem;">
        <div><h1 style="margin:0;">{t['display_name']}</h1><div class="mono" style="color:var(--text-muted); margin-top:0.5rem; font-style:normal;">ID: {t['tg_id'] or t['phone'] or t['tg_username']}</div></div>
        <div style="display:flex; gap:10px;">{alert_btn}<a href="/export/{t_id}" class="btn-outline" style="text-decoration:none;">EXPORT</a><a href="/delete/{t_id}" class="btn-outline" style="text-decoration:none; color:var(--danger); border-color:var(--danger);">DELETE</a></div>
    </div>
    <div style="display:grid; grid-template-columns: repeat(auto-fit, minmax(300px, 1fr)); gap:2rem;">
        <div class="table-wrap">
            <div style="padding:1rem; border-bottom:1px solid var(--border); font-family:'Cinzel', serif; font-weight:700; font-size:0.7rem; color:var(--accent);">SESSION LOG</div>
            <table><tr><th>START</th><th>END</th><th>DURATION</th></tr>{sess_rows}</table>
        </div>
        <div class="stat-card">
            <h2 style="margin-top:0;">Notes</h2>
            <textarea style="height:150px; margin-bottom:1rem; resize:none;" placeholder="Add intelligence..."></textarea>
            <button class="btn">SAVE</button>
        </div>
    </div>"""
    return await render_template_string(HTML_BASE.replace('{{ CONTENT }}', content), active_page='dashboard')
...
async def add():
    ...
    <div style="max-width:600px; margin:auto;">
        ...
        <div class="subtitle">PROVIDE IDENTIFIERS.</div>
        <div class="stat-card">
            <form method="POST">
                <div class="form-group"><label>TARGET DESIGNATION (NAME)</label><input name="name" required placeholder="SUBJECT ALPHA"></div>
                <div style="border-top:1px solid var(--border); margin: 2rem 0; position:relative;"><span style="position:absolute; top:-10px; left:0; background:var(--surface); padding-right:10px; font-size:0.6rem; color:var(--text-muted); font-family:'Cinzel', serif;">IDENTIFIERS</span></div>
                <div style="display:grid; grid-template-columns: 1fr 1fr; gap:1rem;">
                    ...
                    <div class="form-group"><label style="color:var(--success);">PRIORITY 1: PHONE</label><input name="phone" placeholder="+1..."></div>
                </div>
                <div class="form-group"><label>PRIORITY 2: USERNAME</label><input name="username" placeholder="@target_handle"></div>
                <button class="btn">BEGIN SURVEILLANCE</button>
            </form>
        </div>
    </div>"""
    return await render_template_string(HTML_BASE.replace('{{ CONTENT }}', content), active_page='dashboard')
...
async def logs():
    ...
    rows = "".join([f"<tr><td class='mono' style='font-size:0.85rem;'>{fmt_time(l['timestamp'])}</td><td>{l['display_name'] or 'SYSTEM'}</td><td style='font-weight:600; color:{'var(--success)' if l['event_type'] == 'ONLINE' else 'var(--text-muted)'}'>{l['event_type']}</td></tr>" for l in logs])
    content = f"<h1>Audit Logs</h1><div class='subtitle'>SYSTEM ARCHIVE.</div><div class='table-wrap'><table><tr><th>TIME</th><th>ENTITY</th><th>EVENT</th></tr>{rows}</table></div>"
...
async def verify():
    ...
    content = """<div class="stat-card" style="max-width:400px; margin:auto; margin-top:2rem;"><h2>Verify Identity</h2><form method="POST"><div class="form-group"><label>OTP Code</label><input name="code" required></div><button class="btn">Confirm</button></form></div>"""
    return await render_template_string(HTML_BASE.replace('{{ CONTENT }}', content), active_page='settings')
