...
async def init_db():
    async with await get_db() as db:
        ...
        # Ensure 'phone' column exists (User Request)
...
async def login():
    ...
    <div class="card" style="max-width:400px; margin: 100px auto; text-align:center;">
        ...
        <div style="margin-top:20px; font-size:0.8rem;">
            <a href="/register" style="color:var(--neon-purple);">CREATE ACCOUNT (14 DAY TRIAL)</a>
        </div>
    </div>
    """
...
async def add():
    ...
    if request.method == 'POST':
        ...
        async with await get_db() as db:
            ...
            await db.commit()
        return redirect('/dashboard')
    content = """<div class="card"><h2>ADD TARGET</h2><form method="POST"><label>Name</label><input name="name" required><label>Telegram ID or Username</label><input name="tg_input" required><label>Phone (Optional)</label><input name="phone"><button type="submit">INITIATE TRACKING</button></form></div>"""
    return await render_template_string(HTML_BASE.replace('{{ CONTENT }}', content))
