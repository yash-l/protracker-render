...
ENABLE_REGISTRATION = True  # ADDED: Allow new users to sign up
...
# --- DATABASE SCHEMA ---
# Updated to include 'tg_username' for the requested "User ID/Handle" support
DB_SCHEMA = """
...
# --- COMPLEX UI LAYER (Cyberpunk V3 + Matrix Rain + SFX) ---
HTML_BASE = """
<!DOCTYPE html>
<html lang="en">
<head>
...
# --- DATABASE ENGINE ---
async def get_db():
    db = await aiosqlite.connect('tracker.db')
...
async def init_db():
    async with await get_db() as db:
        ...
        # Checks if 'tg_username' exists in targets table, if not, adds it.
        try:
...
class CyberTracker:
    ...
    async def _check_target(self, db, row):
        ...
        # ADDED: Logic to use Username if ID is missing or 0
        ...
        try:
            ...
            # If we resolved by username but didn't have ID, save the ID now
...
async def dashboard():
    ...
    for t in targets:
        ...
        # ADDED: Display Telegram Username if available
...
async def add_target():
    if request.method == 'POST':
        ...
        # ADDED: Logic to handle Username OR ID
        tg_input = form.get('tg_input')
        tg_id = 0
        tg_username = None

        if tg_input.startswith('@') or not tg_input.isdigit():
            tg_username = tg_input  # Treat as username
        else:
            tg_id = int(tg_input)   # Treat as numeric ID
...
# --- ADDED: EDIT TARGET ROUTE ---
@app.route('/edit/<int:t_id>', methods=['GET', 'POST'])
@login_required
async def edit_target(t_id):
    async with await get_db() as db:
        # Security check: Ensure user owns this target or is admin
