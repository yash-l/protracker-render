...
async def do_setup():
    ...
    cfg.update({'admin_username':f['u'],'admin_password_hash':hash_pwd(f['p']),'is_setup_done':True})
    save_config()
...
async def do_login():
    f=await request.form
    if f['u']==cfg['admin_username'] and hash_pwd(f['p'])==cfg.get('admin_password_hash'):
        session['user']=f['u']
        return redirect('/')
    return redirect('/login')
...
async def auth():
    f=await request.form
    cfg.update({'api_id':int(f['aid']),'api_hash':f['hash'],'phone':f['ph']})
    save_config()
    tg=await get_auth_client()
...
@app.route('/verify')
async def verify():
    return await render_template_string(STYLE+f"""<div class="container"><div class="card"><h2>OTP</h2><form action="/do_verify" method="post">{csrf()}<input name="c" type="number" class="input" placeholder="Code" required><br><br><button class="btn">Link</button></form></div></div>"""+FOOTER)
...
async def do_verify():
    ...
    try:
        ...
        cfg['session_string']=tg.session.save()
        save_config()
        await reset_clients()
        return redirect('/')
...
@app.route('/2fa')
async def two_fa():
    return await render_template_string(STYLE+f"""<div class="container"><div class="card"><h2>2FA Required</h2><form action="/do_2fa" method="post">{csrf()}<input type="password" name="pw" class="input" placeholder="Cloud Password" required><br><br><button class="btn">Unlock</button></form></div></div>"""+FOOTER)
...
async def home():
    ...
    async with db.execute("SELECT * FROM targets ORDER BY CASE WHEN current_status='online' THEN 0 ELSE 1 END, last_seen DESC LIMIT 50 OFFSET ?", (offset,)) as c:
...
async def target(uid):
    db = await get_db()
    db.row_factory = aiosqlite.Row
    async with db.execute('SELECT * FROM targets WHERE user_id=?',(uid,)) as c:
        t=await c.fetchone()
...
async def add():
    if request.method=='GET':
        return await render_template_string(STYLE+f"""<div class="container"><div class="card"><h2>Track</h2><form method="post">{csrf()}<input name="t" class="input" placeholder="User/+91.." required><br><br><input name="n" class="input" placeholder="Alias"><br><br><button class="btn">Add</button></form></div></div>"""+FOOTER)
    f=await request.form
    tg=await get_auth_client()
    try:
        if f['t'].strip().isdigit() or f['t'].startswith('+'):
            c=InputPhoneContact(0,f['t'],f['n'],"")
            r=await tg(ImportContactsRequest([c]))
            e=r.users[0]
        else:
            e=await tg.get_entity(f['t'])
        pic=await download_pic(e,tg)
        db = await get_db()
        await db.execute('INSERT OR IGNORE INTO targets (user_id,username,display_name,current_status,last_seen,pic_path) VALUES (?,?,?,?,?,?)',(e.id,getattr(e,'username',''),f['n'] or f['t'],'Scanning',now_iso(),pic))
        await db.commit()
        return redirect('/')
    except:
        return "Error"
...
async def export(uid):
    ...
    si=io.StringIO()
    cw=csv.writer(si)
    cw.writerow(['ID','User','Status','Start','End','Dur'])
    [cw.writerow([r[0],r[1],r[2],fmt_full(r[3]),fmt_full(r[4]),r[5]]) for r in rows]
...
@app.route('/reset')
async def reset():
    return await render_template_string(STYLE+f"""<div class="container"><div class="card"><h2>Reset</h2><form action="/do_reset" method="post">{csrf()}<input name="k" class="input" placeholder="Key" required><br><br><button class="btn" style="background:#444">Reset</button></form></div></div>"""+FOOTER)

@app.route('/do_reset', methods=['POST'])
async def do_reset():
    if (await request.form)['k']==cfg['recovery_key']:
        await reset_clients()
